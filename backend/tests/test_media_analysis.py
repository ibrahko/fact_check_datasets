...
class MediaAnalysisAPITests(APITestCase):
    ...
    def test_upload_and_reanalyze(self):
        ...
        self.assertEqual(analyze_response.status_code, status.HTTP_202_ACCEPTED)

        reanalyze_response = self.client.post(f'/api/media/{media_id}/reanalyze/')
        self.assertEqual(reanalyze_response.status_code, status.HTTP_202_ACCEPTED)
