...
class DeepfakeDetector:
    ...
    def detect_image_artifacts(self, image_path):
        ...
        # ELA approximation: recompression difference
        buffer = io.BytesIO()
    ...
    def analyze_video_frames(self, video_path):
        if cv2 is None:
            return {
                'frame_anomaly_score': 0.0,
                'frames_analyzed': 0,
                'warning': 'opencv-python non installé',
            }
        ...
        return {
            'frame_anomaly_score': round(avg, 3),
            'frames_analyzed': len(anomaly_values),
            'raw_frames': total,
        }
    ...
    def detect_ai_generated_text(self, text):
        ...
        # fallback heuristic if model output is empty or malformed
        if not output_text:
